...
public abstract class AbstractParser {
     ...
     * If next parser is set (!= null) - continues parsing according to chain of responsibility.
     * If next parser is not set - returns Composite object according to regex pattern.
    ...
    public TextComposite parse(String parseString) {
        ...
        TextComposite currentTextComposite = new TextComposite(getTextType());
        while (matcher.find()) {
            if (nextParser != null) {
                TextComposite childTextComposite = nextParser.parse(matcher.group());
